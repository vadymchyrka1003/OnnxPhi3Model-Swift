...
//#include <chrono>
//#include <vector>
//#include "NewCreation-Swift.h"
//#include "ort_genai.h"
//#include "ort_genai_c.h"
//
//
//
//const size_t kMaxTokens = 2000;
//
...
- (void)generate:(nonnull NSString*)input_user_question {
  std::vector<long long> tokenTimes;
  tokenTimes.reserve(kMaxTokens);
  ...
  try {
    ...
    if (!self->model) {
      NSLog(@"Creating model...");
      NSString* llmPath = [[NSBundle mainBundle] resourcePath];
      const char* modelPath = llmPath.UTF8String;
      self->model = OgaModel::Create(modelPath);
    }

    if (!self->tokenizer) {
      NSLog(@"Creating tokenizer...");
      self->tokenizer = OgaTokenizer::Create(*self->model);
    }
